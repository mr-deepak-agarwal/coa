    ...
    <section className="py-24 bg-slate-950 relative" id="interactive">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          <Badge className="mb-4 bg-teal-500/10 text-teal-400 border-teal-500/30 px-4 py-1">
            Hands-on Learning
          </Badge>
          <h2 className="text-4xl sm:text-5xl font-bold text-white mb-4">
            Interactive <span className="bg-gradient-to-r from-teal-400 to-cyan-400 bg-clip-text text-transparent">Lab</span>
          </h2>
        ...
        <Tabs defaultValue="gates" className="w-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <Card className="bg-slate-900/50 border-slate-800">
                ...
                <CardContent className="space-y-6">
                  {/* Gate Selection */}
                  <div>
                    <label className="text-sm text-slate-400 mb-2 block">Select Gate</label>
                  ...
                  <div className="bg-slate-800/50 rounded-lg overflow-hidden">
                    <table className="w-full">
                      ...
                      <tbody className="text-slate-300">
                        {gateType === 'NOT' ? (
                          <>
                            <tr className="border-b border-slate-700/50"><td className="px-4 py-2 text-center">0</td><td className="px-4 py-2 text-center font-semibold text-teal-400">1</td></tr>
                            <tr><td className="px-4 py-2 text-center">1</td><td className="px-4 py-2 text-center font-semibold text-teal-400">0</td></tr>
                          </>
                        ) : (
                          <>
                            <tr className="border-b border-slate-700/50">
                              ...
                              <td className="px-4 py-2 text-center">0</td>
                              <td className="px-4 py-2 text-center font-semibold text-teal-400">
          ...
          <TabsContent value="flipflop">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <Card className="bg-slate-900/50 border-slate-800">
                <CardHeader>
                  <CardTitle className="text-white">Flip-Flop Simulator</CardTitle>
                </CardHeader>
                <CardContent className="space-y-6">
                  {/* FF Type Selection */}
                  <div>
                    <label className="text-sm text-slate-400 mb-2 block">Select Flip-Flop Type</label>
                    <div className="flex flex-wrap gap-2">
                      {['SR', 'D', 'JK', 'T'].map((ff) => (
                        <Button
                          key={ff}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`font-mono text-lg ${ffInputS ? 'text-teal-400' : 'text-slate-500'}`}>
                          {ffInputS ? '1' : '0'}
                        </span>
                        <Switch
                          checked={ffInputS}
                    ...
                    {(flipFlopType === 'SR' || flipFlopType === 'JK') && (
                      <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`font-mono text-lg ${ffInputR ? 'text-teal-400' : 'text-slate-500'}`}>
                            {ffInputR ? '1' : '0'}
                          </span>
                          <Switch
                            checked={ffInputR}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-6 bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border border-slate-700 text-center">
                      ...
                      <span className={`text-5xl font-bold font-mono ${
                        ffOutput ? 'text-teal-400' : 'text-slate-600'
                      }`}>
                        {ffOutput ? '1' : '0'}
                      </span>
                    </div>
              ...
              <Card className="bg-slate-900/50 border-slate-800">
                <CardHeader>
                  <CardTitle className="text-white">{flipFlopType} Flip-Flop Details</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="bg-slate-800/50 rounded-lg p-4">
                    <h4 className="text-teal-400 font-semibold mb-2">Characteristic Equation</h4>
                    <code className="text-xl text-slate-300">
                  ...
                  <div className="bg-slate-800/50 rounded-lg p-4">
                    <h4 className="text-teal-400 font-semibold mb-2">Description</h4>
                  ...
                  <div className="bg-slate-800/50 rounded-lg p-4">
                    <h4 className="text-teal-400 font-semibold mb-2">Applications</h4>
          ...
          <TabsContent value="converter">
            <div className="max-w-2xl mx-auto">
              <Card className="bg-slate-900/50 border-slate-800">
                <CardHeader>
                  <CardTitle className="text-white text-center">Number System Converter</CardTitle>
                </CardHeader>
                <CardContent className="space-y-6">
                  {/* Decimal Input */}
                  <div>
                    <label className="text-sm text-slate-400 mb-2 block">Enter Decimal Number</label>
                    <input
                      type="number"
                      value={decimalInput}
                  ...
                  <div className="bg-slate-800/30 rounded-xl p-6 border border-slate-700">
                    <h4 className="text-slate-400 text-sm mb-4">Binary Breakdown (8-bit)</h4>
                    <div className="flex justify-center gap-1">
                      {decToBinary(decimalInput).split('').map((bit, idx) => (
                        <div key={idx} className="flex flex-col items-center">
                          ...
                          <span className="text-xs text-slate-500 mt-1">2^{7 - idx}</span>
                        </div>
                      ))}
                    </div>
                  </div>
